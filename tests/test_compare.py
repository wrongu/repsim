...
def test_riemmannian_rank_deficient_inf_distance(data_x, data_y):
    assert size_n < size_m, "Test constants changed?! Rank deficiency test assumes n < m"
    ...
    assert torch.isinf(compare(data_x, data_y, method=unregularized, kernel_x=Linear(), kernel_y=Linear()))
