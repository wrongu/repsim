...
def test_geodesic_stress():
    x, y = torch.randn(5, 3, dtype=torch.float64), torch.randn(
...
def _test_project_helper(x, y, z, metric):
    ...
    assert np.isclose(
        dist_xy, dist_xp + dist_py, atol=tolerance
