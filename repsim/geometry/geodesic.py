...
def midpoint(space: "LengthSpace", pt_a: "Point", pt_b: "Point", **kwargs) -> "Point":
    ...
    :return: pt_c : the midpoint between pt_a and pt_b such that space.length(pt_a,pt_c)==space.length(pt_c,pt_b)
    """
    return space.geodesic(pt_a, pt_b, frac=0.5, **kwargs)


def subdivide_geodesic(space: "LengthSpace", pt_a: "Point", pt_b: "Point", octaves: int = 5, **kwargs) -> List["Point"]:
    """Compute multiple points along geodesic from pt_a to pt_b by recursively halving 'octaves' times. Result is a list
    of points where index [0] is pt_a and index [-1] is pt_b.
