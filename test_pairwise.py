...
def test_compare_random_data():
    ...
    for k in kernels:
        for cmp in CompareType:
            ...
            assert not torch.any(torch.isnan(xx)), f"NaN value in pairwise comparison using {cmp}!"
            assert torch.allclose(xx, xx.T), f"Asymmetric pairwise comparison using {cmp}"
